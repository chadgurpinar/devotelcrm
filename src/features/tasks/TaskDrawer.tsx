      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-gray-200 bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><label className="mb-1 block text-xs font-medium text-gray-500">Status</label><select value={draft.status} onChange={(e) => setDraft((p) => ({ ...p, status: e.target.value as TaskStatus }))} className={inputCls}><option value="Backlog">Backlog</option><option value="Open">Open</option><option value="InProgress">In Progress</option><option value="Done">Done</option><option value="Completed">Completed</option><option value="Archived">Archived</option></select></div>
        ...
        <div className="sticky bottom-0 flex items-center justify-between border-t border-gray-100 bg-white px-6 py-4 rounded-b-2xl">
          <div className="flex gap-2">
            {task.status !== "Done" && task.status !== "Completed" && task.status !== "Archived" && <button onClick={() => onArchive(task)} className="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-50 transition">⭐ Star</button>}
            {(task.status === "Archived" || (task.status === "Done" && task.archivedAt)) && <button onClick={() => onUnarchive(task)} className="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-50 transition">Unstar</button>}
          </div>
          <div className="flex gap-2">
