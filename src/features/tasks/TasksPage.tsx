...
const STATUS_BADGE: Record<string, string> = { Backlog: "bg-gray-100 text-gray-600", Open: "bg-blue-50 text-blue-700", InProgress: "bg-indigo-50 text-indigo-700", Done: "bg-emerald-50 text-emerald-700", Completed: "bg-emerald-100 text-emerald-800", Archived: "bg-amber-50 text-amber-700" };
...
export function TasksPage() {
  ...
  useEffect(() => { if (isDoneSection) { setViewModePersisted("LIST"); if (statusFilter === "Open" || statusFilter === "InProgress") setStatusFilter("Any"); return; } if (statusFilter === "Done" || statusFilter === "Completed" || statusFilter === "Archived") setStatusFilter("Any"); }, [isDoneSection, statusFilter, setViewModePersisted]);
  const isTerminalStatus = (s: string) => s === "Completed" || s === "Archived";

  const sectionTasks = useMemo(() => { if (section === "Completed") return state.tasks.filter((t) => t.status === "Completed"); if (section === "Archive") return state.tasks.filter((t) => t.status === "Archived"); if (section === "AssignedToMe") return state.tasks.filter((t) => !isTerminalStatus(t.status) && t.assigneeUserId === state.activeUserId && t.createdByUserId !== state.activeUserId); if (section === "AssignedByMe") return state.tasks.filter((t) => !isTerminalStatus(t.status) && t.createdByUserId === state.activeUserId && t.assigneeUserId !== state.activeUserId); return state.tasks.filter((t) => !isTerminalStatus(t.status) && t.createdByUserId === state.activeUserId && t.assigneeUserId === state.activeUserId); }, [section, state.activeUserId, state.tasks]);

  const rows = useMemo(() => { let d = sectionTasks; if (statusFilter !== "Any") d = d.filter((t) => t.status === statusFilter); if (priorityFilter !== "Any") d = d.filter((t) => t.priority === priorityFilter); if (dueFilter === "DueSoon") d = d.filter((t) => isDueSoon(t.dueAt)); if (dueFilter === "Overdue") d = d.filter((t) => isOverdue(t)); if (dueFilter === "NoDueDate") d = d.filter((t) => !t.dueAt); if (linkedFilter === "Company") d = d.filter((t) => Boolean(t.companyId)); if (linkedFilter === "Event") d = d.filter((t) => Boolean(t.eventId)); if (linkedFilter === "Interconnection") d = d.filter((t) => Boolean(t.interconnectionProcessId)); if (linkedFilter === "Project") d = d.filter((t) => Boolean(t.projectId)); if (labelFilter !== "Any") d = d.filter((t) => (t.labelIds ?? []).includes(labelFilter)); if (search.trim()) { const q = search.trim().toLowerCase(); d = d.filter((t) => { const cm = commentsByTaskId.get(t.id) ?? []; return t.title.toLowerCase().includes(q) || t.description.toLowerCase().includes(q) || cm.some((c) => c.content.toLowerCase().includes(q)); }); } return d.slice().sort((a, b) => { if (a.isUrgent && !b.isUrgent) return -1; if (!a.isUrgent && b.isUrgent) return 1; if (sortBy === "DueDateAsc") { if (!a.dueAt && !b.dueAt) return 0; if (!a.dueAt) return 1; if (!b.dueAt) return -1; return a.dueAt.localeCompare(b.dueAt); } if (sortBy === "CreatedDateDesc") return b.createdAt.localeCompare(a.createdAt); if (sortBy === "PriorityDesc") return priorityWeight[b.priority] - priorityWeight[a.priority]; return b.updatedAt.localeCompare(a.updatedAt); }); }, [commentsByTaskId, dueFilter, labelFilter, linkedFilter, priorityFilter, search, sectionTasks, sortBy, statusFilter]);

  function handleCreateTask() { const title = form.title.trim(); if (!title) return; const lf = form.linkedType === "Company" ? { companyId: form.linkedId || undefined } : form.linkedType === "Event" ? { eventId: form.linkedId || undefined } : form.linkedType === "Interconnection" ? { interconnectionProcessId: form.linkedId || undefined } : form.linkedType === "Project" ? { projectId: form.linkedId || undefined } : {}; state.createTask({ title, description: form.description.trim(), status: "Backlog", priority: form.priority, dueAt: form.dueAt ? new Date(`${form.dueAt}T12:00:00`).toISOString() : undefined, createdByUserId: state.activeUserId, assigneeUserId: form.assigneeUserId || state.activeUserId, visibility: form.visibility, isUrgent: form.isUrgent, kanbanStage: "Backlog", labelIds: form.labelIds.length > 0 ? form.labelIds : undefined, ...lf, initialComment: form.initialComment.trim() || undefined }); setForm(emptyCreateTaskForm(state.activeUserId)); setCreateModalOpen(false); }
    ...
    <div className="space-y-6">
      <UiPageHeader title="Tasks" subtitle="Manage your personal and team tasks" actions={
        <div className="flex items-center gap-2">
          <button onClick={() => setLabelManagerOpen(true)} className="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 transition"><Tag className="h-4 w-4" /> Manage Labels</button>
          <button onClick={() => { setForm(emptyCreateTaskForm(state.activeUserId)); setCreateModalOpen(true); }} className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 transition"><Plus className="h-4 w-4" /> Create Task</button>
        </div>
      } />
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1 rounded-xl bg-gray-100 p-1 w-fit">
          {SECTION_TABS.map((t) => <button key={t.key} onClick={() => setSection(t.key)} className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-all ${section === t.key ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>{t.label}</button>)}
        </div>
        {!isDoneSection && (
        ...
        <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody>
                {rows.length === 0 && <tr><td colSpan={6} className="px-4 py-12 text-center text-sm text-gray-400">{section === "Completed" ? "No completed tasks yet." : section === "Archive" ? "No archived tasks yet." : "No tasks found."}</td></tr>}
                {rows.map((task) => {
                  ...
                  return (
                    <tr key={task.id} className={`border-b border-gray-100 hover:bg-indigo-50/30 transition-colors cursor-pointer ${rowCls}`} onClick={() => setSelectedTaskId(task.id)}>
                      <td className="px-4 py-2"><p className="font-semibold text-gray-900 text-sm">{task.title}</p></td>
                      <td className="px-4 py-2" onClick={(e) => e.stopPropagation()}>
                        <span className={`inline-flex items-center rounded-full px-1 ${STATUS_BADGE[task.status] ?? "bg-gray-100 text-gray-600"}`}>
                          <select value={task.status} onClick={(e) => e.stopPropagation()} onChange={(e) => { e.stopPropagation(); const t = state.tasks.find((t2) => t2.id === task.id); if (t) state.updateTask({ ...t, status: e.target.value as TaskStatus }); }} className="rounded-full border-0 bg-transparent text-xs font-medium px-1.5 py-0.5 cursor-pointer focus:ring-1 focus:ring-indigo-400 focus:outline-none">
                            <option value="Backlog">Backlog</option><option value="Open">Open</option><option value="InProgress">In Progress</option><option value="Done">Done</option><option value="Completed">Completed</option><option value="Archived">Archived</option>
                          </select>
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-2" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center gap-1">
                          {section === "Completed" && <button onClick={(e) => { e.stopPropagation(); state.updateTask({ ...task, status: "Archived" }); }} className="rounded-lg border border-amber-200 bg-amber-50 px-2 py-1 text-[11px] font-medium text-amber-700 hover:bg-amber-100 transition">⭐ Star</button>}
                          {section === "Archive" && <button onClick={(e) => { e.stopPropagation(); state.updateTask({ ...task, status: "Completed", archivedAt: undefined }); }} className="rounded-lg border border-gray-200 bg-white px-2 py-1 text-[11px] font-medium text-gray-600 hover:bg-gray-50 transition">Unstar</button>}
      ...
      {/* Create Task Modal — 2-column layout */}
      {isCreateModalOpen && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4" onClick={() => setCreateModalOpen(false)}>
          <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={() => setCreateModalOpen(false)} className="rounded-lg p-1 text-gray-400 hover:bg-gray-100 transition"><X className="h-5 w-5" /></button>
            </div>
            ...
            {/* Labels — full width multi-select */}
            {state.taskLabels.length > 0 && (
              <div className="mt-4">
                <MultiSelectDropdown items={state.taskLabels} selectedIds={form.labelIds} getItemId={(l) => l.id} getItemLabel={(l) => l.name} getItemColor={(l) => l.color} onToggle={(id) => setForm((p) => ({ ...p, labelIds: p.labelIds.includes(id) ? p.labelIds.filter((i) => i !== id) : [...p.labelIds, id] }))} label="Labels" />
              </div>
            )}
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4" onClick={() => setLabelManagerOpen(false)}>
          <div className="w-full max-w-3xl rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={() => { setLabelManagerOpen(false); setEditingLabelId(null); }} className="rounded-lg p-1 text-gray-400 hover:bg-gray-100 transition"><X className="h-5 w-5" /></button>
            </div>
