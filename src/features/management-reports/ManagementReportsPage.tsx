...
function WeekNav({ week, onPrev, onNext }: { week: string; onPrev: () => void; onNext: () => void }) {
  return (<div className="flex items-center gap-2"><button onClick={onPrev} className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 transition"><ChevronLeft className="h-4 w-4" /></button><span className="text-sm font-medium text-gray-700">Week of {formatWeekLabel(week)}</span><button onClick={onNext} className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 transition"><ChevronRight className="h-4 w-4" /></button></div>);
}
      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-gray-200 bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-5 space-y-6">
          <div className="grid grid-cols-4 gap-3">
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 text-center"><p className="text-xs text-gray-500 mb-1">Submit Rate</p><p className="text-lg font-bold text-gray-900">{submissionRate}%</p><p className="text-[10px] text-gray-400">last 12 weeks</p></div>
          ...
          {empUser && (
            <div><p className="mb-2 text-xs font-semibold text-gray-700">Active Tasks ({openTasks.length})</p>
              {openTasks.length === 0 ? <p className="text-xs text-gray-400">No open tasks.</p> : (
                <div className="space-y-1 max-h-40 overflow-y-auto">{openTasks.slice(0, 10).map((task) => { const isOd = task.dueAt && new Date(task.dueAt) < new Date() && task.status !== "Done" && task.status !== "Completed"; return (<div key={task.id} className="flex items-center justify-between rounded-lg border border-gray-100 bg-gray-50 px-3 py-1.5 text-xs"><span className={`font-medium ${isOd ? "text-rose-700" : "text-gray-700"} truncate max-w-[300px]`}>{isOd ? "⚠ " : ""}{task.title}</span><div className="flex items-center gap-2 flex-shrink-0"><span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${task.priority === "Critical" ? "bg-rose-50 text-rose-700" : task.priority === "High" ? "bg-orange-50 text-orange-700" : "bg-gray-100 text-gray-600"}`}>{task.priority}</span><span className="text-gray-400">{task.status === "InProgress" ? "In Progress" : task.status}</span></div></div>); })}{openTasks.length > 10 && <p className="text-[10px] text-gray-400 text-center">+{openTasks.length - 10} more</p>}</div>
              )}
              <div className="mt-2 flex items-center gap-3 text-[11px] text-gray-500"><span>✓ {completedTasks.length} completed</span>{overdueTasks.length > 0 && <span className="text-rose-600">⚠ {overdueTasks.length} overdue</span>}</div>
            </div>
          )}
          <div><p className="mb-2 text-xs font-semibold text-gray-700">Report History (Last 12 Weeks)</p>
            <div className="space-y-1.5 max-h-64 overflow-y-auto">{[...empReports].reverse().map(({ week, report }) => (
              <div key={week} className="rounded-lg border border-gray-100 bg-gray-50 px-3 py-2">
                <div className="flex items-center justify-between"><span className="text-xs text-gray-700">{formatWeekLabel(week)}</span><div className="flex items-center gap-3">{report && <><span className={`text-[11px] ${wlColor(report.workloadRating)}`}>WL: {WORKLOAD_LABELS[report.workloadRating]}</span><span className={`text-[11px] ${prColor(report.productivityRating)}`}>PR: {PRODUCTIVITY_LABELS[report.productivityRating]}</span></>}<StatusPill status={report?.status === "Submitted" ? "Submitted" : report ? "Draft" : "missing"} /></div></div>
                {report?.reportText && <p className="mt-1 text-[11px] text-gray-500 line-clamp-2">{report.reportText}</p>}
                {report?.highlights && report.highlights.length > 0 && <div className="flex flex-wrap gap-1 mt-1">{report.highlights.map((h, i) => <span key={i} className="rounded-full bg-white border border-gray-200 px-2 py-0.5 text-[10px] text-gray-600">{h}</span>)}</div>}
              </div>
            ))}</div>
          </div>
        </div>
