...
function buildCounts(db: DbState): Record<keyof DbState, number> {
  return {
    ...
    hrCompChangeLogs: db.hrCompChangeLogs.length,
    hrPublicHolidays: db.hrPublicHolidays.length,
    hrDigitalSignatures: db.hrDigitalSignatures.length,
    ...
    routingNocRequests: db.routingNocRequests.length,
    amEntries: db.amEntries.length,
    nocMembers: db.nocMembers.length,
    nocPerfWeekEntries: db.nocPerfWeekEntries.length,
    nocPerfMonthSummaries: db.nocPerfMonthSummaries.length,
    ...
    wholesaleTrafficRecords: db.wholesaleTrafficRecords.length,
    trafficBaselines: db.trafficBaselines.length,
    trafficAlertRules: db.trafficAlertRules.length,
    trafficAlertEvents: db.trafficAlertEvents.length,
    ...
    financeBankAccounts: db.financeBankAccounts.length,
    financeLiquidityThresholds: db.financeLiquidityThresholds.length,
    financeCreditCardTransactions: db.financeCreditCardTransactions.length,
    financeSalaryDefaults: db.financeSalaryDefaults.length,
    financeContractors: db.financeContractors.length,
    financeForecastSnapshots: db.financeForecastSnapshots.length,
    financeBankConnections: db.financeBankConnections.length,
