...
export function generateSeedDb(
  ...
  const db: DbState = {
    ...
    routingNocRequests: (() => {
      ...
      const ago = (m: number) => new Date(now - m * 60 * 1000).toISOString();
      return [
        { id: "rnr-01", tab: "Routing Request" as const, fields: { Customer: "Vodafone DE", Destination: "Germany", "Provider 1": "BT Wholesale", "Provider 2": "Tele2", "Provider 3": "", "Provider 4": "", "Provider 5": "", Comment: "Please optimise ASR for DE traffic" }, submittedBy: "Sarah Mitchell", submittedAt: ago(45), status: "Open" as const },
        { id: "rnr-02", tab: "Routing Request" as const, fields: { Customer: "Orange FR", Destination: "France", "Provider 1": "Syniverse", "Provider 2": "BICS", Comment: "" }, submittedBy: "James Carter", submittedAt: ago(180), status: "Routing Done" as const, closedBy: "Ali Yılmaz", closedAt: ago(120), nocComment: "Re-routed via BICS, ASR improved to 74%", reviewedByAm: false },
        { id: "rnr-03", tab: "Routing Request" as const, fields: { Customer: "T-Mobile US", Destination: "United States", "Provider 1": "Telia Carrier", "Provider 2": "", Comment: "ASR below 50% on current route" }, submittedBy: "Elif Demir", submittedAt: ago(320), status: "Routing Done" as const, closedBy: "Mehmet Kaya", closedAt: ago(280), nocComment: "Switched to Telia direct route, ASR now 68%", reviewedByAm: true },
        { id: "rnr-04", tab: "TT Request" as const, fields: { Customer: "Turkcell", Destination: "Turkey", Issue: "High failure rate on MT SMS last 2 hours", Comment: "Urgent please" }, submittedBy: "Elif Demir", submittedAt: ago(25), status: "Open" as const },
        { id: "rnr-05", tab: "TT Request" as const, fields: { Customer: "MTN Nigeria", Destination: "Nigeria", Issue: "DLR drop below 60% since 11:00 UTC", Comment: "" }, submittedBy: "James Carter", submittedAt: ago(200), status: "TT Sent" as const, closedBy: "Mehmet Yıldız", closedAt: ago(160), nocComment: "TT raised with provider, ref: TT-2024-9921", reviewedByAm: true },
        { id: "rnr-06", tab: "TT Request" as const, fields: { Customer: "Etisalat", Destination: "UAE", Issue: "Complete delivery failure on A2P routes", Comment: "Customer escalated" }, submittedBy: "Sarah Mitchell", submittedAt: ago(90), status: "TT Sent" as const, closedBy: "Ali Yılmaz", closedAt: ago(60), nocComment: "TT-2026-0401 raised, provider acknowledged", reviewedByAm: false },
        { id: "rnr-07", tab: "Test Request" as const, fields: { Customer: "Zain KSA", Destination: "Saudi Arabia", "Test Type": "MT SMS delivery test", Comment: "Need results ASAP" }, submittedBy: "Rania Al-Hassan", submittedAt: ago(35), status: "Open" as const },
        { id: "rnr-08", tab: "Test Request" as const, fields: { Customer: "Globe PH", Destination: "Philippines", "Test Type": "Voice call completion test", Comment: "" }, submittedBy: "James Carter", submittedAt: ago(150), status: "Test Successful" as const, closedBy: "Mehmet Kaya", closedAt: ago(110), nocComment: "All 10 test calls connected, avg duration 45s", reviewedByAm: true },
        { id: "rnr-09", tab: "Test Request" as const, fields: { Customer: "Reliance Jio", Destination: "India", "Test Type": "A2P SMS delivery test", Comment: "New route validation" }, submittedBy: "Priya Nair", submittedAt: ago(240), status: "Test Failed" as const, closedBy: "Ali Yılmaz", closedAt: ago(200), nocComment: "3/10 messages delivered, route not viable", reviewedByAm: false },
        { id: "rnr-10", tab: "Loss Accepted" as const, fields: { Customer: "Airtel India", Destination: "India", "Acceptable Loss Value": "5%", Comment: "Seasonal traffic spike" }, submittedBy: "Priya Nair", submittedAt: ago(15), status: "Open" as const },
        { id: "rnr-11", tab: "Loss Accepted" as const, fields: { Customer: "Claro Brazil", Destination: "Brazil", "Acceptable Loss Value": "12%", Comment: "" }, submittedBy: "Sarah Mitchell", submittedAt: ago(300), status: "Loss Not Accepted" as const, closedBy: "Ali Yılmaz", closedAt: ago(260), nocComment: "12% is above threshold, max 8% acceptable", reviewedByAm: false },
        { id: "rnr-12", tab: "Loss Accepted" as const, fields: { Customer: "Singtel", Destination: "Singapore", "Acceptable Loss Value": "3%", Comment: "Within tolerance" }, submittedBy: "James Carter", submittedAt: ago(180), status: "Loss Accepted" as const, closedBy: "Mehmet Kaya", closedAt: ago(150), nocComment: "3% within acceptable range, approved", reviewedByAm: true },
      ];
    })(),
