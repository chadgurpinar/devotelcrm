...
export function generateSeedDb(
  ...
  const db: DbState = {
    ...
    amEntries: (() => {
      ...
      const future = (h: number) => new Date(now + hrs(h)).toISOString();
      const past = (h: number) => new Date(now - hrs(h)).toISOString();
      const c = (id: string, text: string, authorName: string, hoursAgo: number): { id: string; text: string; authorName: string; createdAt: string } =>
        ({ id, text, authorName, createdAt: past(hoursAgo) });
      return [
        { id: "am-01", tab: "Route Request" as const, fields: { Destination: "Nigeria MTN", "Target Rate": "7.5", Volume: "10K/day" }, submittedBy: "Sarah Mitchell", submittedAt: past(6), expiresAt: future(42), isArchived: false, comments: [c("amc-01", "BT Wholesale has good rates for MTN, I can connect you", "James Carter", 4), c("amc-02", "Syniverse also competitive here, check their portal", "Timur Aslan", 2)] },
        { id: "am-02", tab: "Route Request" as const, fields: { Destination: "Germany Telekom", "Target Rate": "12.0", Volume: "50K/day" }, submittedBy: "Ceren Yıldız", submittedAt: past(3), expiresAt: future(45), isArchived: false, comments: [] },
        { id: "am-03", tab: "Traffic Request" as const, fields: { Destination: "Turkey Turkcell", Rate: "9.0", Volume: "25K/day" }, submittedBy: "Elif Demir", submittedAt: past(10), expiresAt: future(38), isArchived: false, comments: [c("amc-03", "We have capacity on Turkcell, let's discuss", "Rania Al-Hassan", 8)] },
        { id: "am-04", tab: "Traffic Request" as const, fields: { Destination: "India Airtel", Rate: "5.5", Volume: "100K/day" }, submittedBy: "Timur Aslan", submittedAt: past(1), expiresAt: future(47), isArchived: false, comments: [] },
        { id: "am-05", tab: "Targets" as const, fields: { Destination: "France Orange", "Target Rate": "11.0", Volume: "30K/day" }, submittedBy: "James Carter", submittedAt: past(12), expiresAt: future(36), isArchived: false, comments: [] },
        { id: "am-06", tab: "Deal Offers" as const, customer: { Customer: "Vodafone UK", Destination: "UK", Rate: "15.0", Volume: "20K/day", "Deal Period": "3 months", Amount: "$45,000" }, provider: { Provider: "BICS", Destination: "UK", Rate: "13.5", Volume: "20K/day", "Deal Period": "3 months", Amount: "$38,000" }, submittedBy: "Sarah Mitchell", submittedAt: past(5), expiresAt: future(43), isArchived: false, comments: [c("amc-04", "Margin looks good, I support this deal", "Ceren Yıldız", 3)] },
        { id: "am-07", tab: "Route Request" as const, fields: { Destination: "Brazil Claro", "Target Rate": "6.0", Volume: "15K/day" }, submittedBy: "Rania Al-Hassan", submittedAt: past(72), expiresAt: past(24), isArchived: true, comments: [] },
        { id: "am-08", tab: "Deal Offers" as const, customer: { Customer: "MTN SA", Destination: "South Africa", Rate: "8.0", Volume: "40K/day", "Deal Period": "6 months", Amount: "$120,000" }, provider: { Provider: "Telia Carrier", Destination: "South Africa", Rate: "6.8", Volume: "40K/day", "Deal Period": "6 months", Amount: "$98,000" }, submittedBy: "Elif Demir", submittedAt: past(60), expiresAt: past(12), isArchived: true, comments: [c("amc-05", "Deal expired, provider did not respond in time", "Timur Aslan", 15)] },
      ];
    })(),
